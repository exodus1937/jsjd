...
function clickOpenWindow(code,name,time_before,time_after,methods,points){ 
	...
	for(var i = 0;i<codes.length;i++){
		if(codes[i] != ""){
			var $div = $("<div class='info"+i+"' style='float:left;height:20px;color:#58595B;font-style:normal'>"+names[i]+"的计算方法:"+m[i]+"，涉及测点"+p[i]+"<a href='#' id='delete"+i+"' onclick='deleteDiv("+i+")'> 删</a></div><br/>")
			//var $div1 =$("<div class='info"+i+"' style='float:left;height:20px'>"+names[i]+"的计算方法:"+methods+"</div>")
						 // <div class="info1" style="float: left; height: 20px;">"+names[i]+"+"+methods+"</div>
			  ...
			  success: function(data){
				  ...
				  for(i=0;i<ev.length;i++){
					  ...
					  spanHtml += "<a href='javascript:void(0)' onclick=clickFunc('"+name+"','"+ev[i].NAVIGATION_ID+"','"+ev[i].APPLICATION_URL+"') style='color: #000000;' >"
						+"<div style='background:url("+ev[i].IMAGE_URL+")  no-repeat center 10px #2fa9cc;float: left;margin-top:7%;' id='"+b+"'>"
							+"<h3 style='margin-top:65px;text-align:center;font-family: 微软雅黑; color:#ffffff;font-size:10pt;'>"+name+"</h3>"
						+"</div></a>";
				  }
