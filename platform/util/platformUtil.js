...
function showExtWindow(config){
	...
	var extWindow = new Ext.Window({
		...
		html: '<iframe id="extWindowIframe" style="overflow:auto;width:100%; height:100%;" src="' + winUrl + '" frameborder="0"></iframe>',
		listeners: {
			show: function(win) {},
			close: function() {
 ...
 */
function showAttachmentManager(userId, src_id, org_id, att_cat, bus_no, canUp, canDown, canDel, closeCallback, fileSizeLimit){
...
//	var url = contextPath + '/main?xwl=23WJH4A46NYK'
				+ '&userId=' + userId
		        + '&srcId=' + src_id
		        + '&orgId=' + org_id
		        + '&attCat=' + att_cat
...
function exportExcel(templateCode, sqlParams, excelName){
	...
	window.location.href = contextPath + "/templateManageAction.do?method=exportExcel&templateCode="+templateCode+"&sqlParams="+sqlParams+"&excelName="+excelName;
	return;
}
...
/*$("head").prepend('<meta http-equiv="x-ua-compatible" content="IE=Emulate8" />');*/