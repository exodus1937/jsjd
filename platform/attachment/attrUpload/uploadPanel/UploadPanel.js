...
Ext.define('Ext.ux.uploadPanel.UploadPanel',{
	...
	columns : [
		...
		}},
        {
            ...
            width:50,
            items: [{
                icon: 'platform/attachment/attrUpload/uploadPanel/icons/delete.gif',
                tooltip: 'Remove',
	...
	upload_success_handler : function(file, serverData, responseReceived){
		...
		if(!Ext.JSON.decode(serverData).success){			
			Ext.Msg.alert('',Ext.JSON.decode(serverData).msg);		
		}
	},
	...
	onUpload : function(){
		if (this.swfupload&&this.store.getCount()>0) {
			...
			Ext.Ajax.request({
			  url : this.getFileNamesUrl,
			  async:false,
			  //params:{"src_id":this.post_params.src_id},
			  method: 'POST',
			...
			if(fileNames!=null){
				for(var i=0; i<fileNames.length; i++){
					if(this.store.find('fileName',fileNames[i].ATT_FILE_NAME) > -1){
						flag = true;
						msg = '文件【' + fileNames[i].ATT_FILE_NAME + '】已存在,不能够重复上传！';
						break;
					}
				}
			}
