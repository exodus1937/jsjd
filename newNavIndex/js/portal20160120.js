...
function changeView(){
	getTargetDateValues();//执行集团视图的ajax获取数据
...
$(".title").on("click",function(){
	...
	if($(this).find('a').html()=='运行机组报警' || $(this).find('a').html()=="停机机组报警"){
		//console.log('aaa')
		baojinggenduo.href="http://10.10.10.32:7001/jsjd/main?xwl=23WPD5TO7GWR"
	}
	if($(this).find('a').html()=='预警通知单'){
		baojinggenduo.href="http://10.10.10.32:7001/jsjd/main?xwl=23WPD5TO7GWT"
	}
				  ...
				  if(position=="1"){					 				 
					  ...
					  for(i=0;i<ev.length;i++){
						  ...
						  if(typeof(ev[i].VALUEDATE)!='undefined'){
					           ...
					           spanHtml+="<div class='td1' style='height:32px;'>"
										   +"<span class='tdn'>" +
