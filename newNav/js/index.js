...
function showIndex(url,menuName){
	...
	if(url == "http://172.168.100.101/Coresight"){
		window.open(url);
		//$("#index_page").attr("src","");
	}else{
...
function fixPosition(parent,child){
	var top;
	var pof = parent.context.offsetTop;//parent offsetheight;
	...
	/*if(ch>=500){
		top=pof-ch;
		//console.log("大于500px  top：="+top )
	}	*/
	if(ch==442){
		top=pof-ch/2-17+34*2;
	}
	...
	//console.log("parent offsetTop"+pof);
	child.parent().css({top:top,left:194});

}
if (!window.location.origin) {
	window.location.origin = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port : '');
}
var ctx_new = window.location.origin;
$("#fangke").click(function(){
	
	click_a(ctx_new+"/jsjd/fangke/html/fangke.html");
});

function fangke(){
	$.ajax({
		url: ctx_new+"/jsjd/newPage.do?method=monitor",
		success:function(data){
			$("#fangke").html("<div>当前在线："+data.total+"&nbsp;人</div><div>累计访问："+data.userCount+"&nbsp;次</div>")
		}
	})
}
fangke();