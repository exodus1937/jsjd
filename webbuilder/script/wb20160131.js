var Wb = {
	...
	getExcel : function(grid, isAll, preview) {
			...
			for (i in cs) {
				...
				if (c.type == 'rowNumber') {
					...
					x = {
						...
						wrap : c.autoWrap,
						text : encodeURIComponent(c.text == '&#160;' ? '' : c.text)
					};
					if (c.keyName)
		...
		Ext.Object.each(a.allParams,function(key,value){
			if(Ext.isDate(value)){
                a.allParams[key]=encodeURIComponent(Wb.dateToStr(value, false));
            }else{
                a.allParams[key]=encodeURIComponent(value||'');
            }
		});
		...
		if (g.exportTitle == '-')
			...
			p.xwl_title = encodeURIComponent(g.exportTitle || g.title);
		p.xwl_file = encodeURIComponent(g.exportFilename || p.xwl_title);
		...
		x.win = new Ext.window.Window(
				{
					...
					listeners : {
						beforeshow : function(w) {
							...
							if (f)
								xwlw__x_img.setSrc('main?xwl=13MC0NFI4FRE&'
										+ Wb.getId());
					...
					items : [
							...
							},
							{
								id : 'xwlw__x_rfl',
								width : 56,
								html : '<a href="javascript:xwlw__x.changeVC()">'
										+ Str.change + '</a>',
								x : 296,
								xtype : 'label',
								y : 144 + 4
	...
	// 兴竹开发集成平台添加调用数据库存储过程
	executeProcedure : function(appCode, procedureName, params, callback) {
		Ext.Ajax.request({
			url : "platformAction.do?method=executeProcedure",
			method : 'POST',
			timeout : 300000,
			async : false,
			params : {
	...
	getChart : function(chart) {
		Wb.download('main?xwl=13NUIG6TAFSD',
				{
					data : Ext.draw.engine.SvgExporter.self.generate({},
							chart.surface),
					file : 'chart.svg'
				}, true);
	},
			...
			for (n in value) {
				...
				if (obj instanceof Ext.form.CheckboxGroup) {
					...
					if (reset)
						...
						if (obj instanceof Ext.form.field.ComboBox && obj.store)
							obj.store.clearFilter(true);
						obj.setValue(v);
					}
				} else if (obj instanceof Ext.grid.Panel) {
					if (reset && obj.store)
