var Ds = {
	...
	objectTreeFields : [ 'text', 'id', 'xwlMeta', 'xwlChildren', 'xwlParent',
			'xwlXtype', 'xwlType', 'xwlCategory', 'xwlWidth', 'xwlHeight',
			'xwlMinWidth', 'xwlMinHeight', 'xwlMaxWidth', 'xwlMaxHeight',
			'xwlPT', 'properties', 'events', 'custPEditors', 'custEEditors',
			'custEPara' ],
	...
	monitorKey : function(e, o) {
		...
		if (e.ctrlKey) {
			...
			if (!Ds.moduleSeled && at && at.saveUrl)
				return;
			Ds.keyCode = keyCode;
			if (k == 'C'
	...
	addSE : function(obj, type) {
		...
		page
				.add( {
					iconCls : type == 'html' ? 'web_icon' : type + '_icon',
					...
					listeners : {
						render : function(t) {
							...
							var b = type == 'js' || type == 'ss', editorCfg, rtype;
							switch (type) {
							case 'js':
							case 'ss':
								rtype = 'text/javascript';
								break;
							case 'sql':
								rtype = 'text/x-sql';
								break;
							...
							editorCfg = {
								lineNumbers : true,
								matchBrackets : b || type == 'sql',
								theme : Ds.editorTheme || 'default',
								extraKeys : {
									'Ctrl-/' : 'toggleComment',
									'Ctrl-.' : function() {
										if (scriptEditor.modifyCursor)
											scriptEditor
													.setCursor(scriptEditor.modifyCursor);
									},
									'Shift-Ctrl-F' : function() {
										var totalLines = scriptEditor
												.lineCount();
										var totalChars = scriptEditor
												.getTextArea().value.length;
										var c = scriptEditor.getCursor();
										scriptEditor.autoFormatRange( {
											line : 0,
											ch : 0
										}, {
											line : totalLines,
							...
							if (b) {
								editorCfg.extraKeys['Alt-/'] = 'autocomplete';
								editorCfg.extraKeys['.'] = function() {
									scriptEditor.replaceSelection('.');
									clearTimeout(scriptEditor.hintTimer);
									scriptEditor.hintTimer = setTimeout(
											function() {
												CodeMirror
														.showHint(scriptEditor);
											}, 100);
								};
								editorCfg.gutters = [ "CodeMirror-lint-markers" ];
								editorCfg.lint = true;
							}
							scriptEditor = CodeMirror.fromTextArea(Wb
									.dom(edtId), editorCfg);
							scriptEditor.xwlNodes = type == 'js';
							scriptEditor.editorType = type;
							scriptEditor.on("change", function() {
								if (scriptEditor.sttChg) {
									scriptTabObj.isModified = true;
									scriptEditor.modifyCursor = Ext.apply( {},
											scriptEditor.lastCursor);
									Ds.setModified();
								}
							});
							scriptEditor.on("cursorActivity", function() {
								var o = scriptEditor.getCursor();
								scriptEditor.lastCursor = o;
						...
						beforedeactivate : function(t) {
							if (Ext.isIE)
								scriptEditor.saveCursor = scriptEditor
										.getScrollInfo();
						},
						activate : function(t) {
							if (Ext.isIE && scriptEditor.saveCursor)
								scriptEditor.scrollTo(
										scriptEditor.saveCursor.left,
										scriptEditor.saveCursor.top);
							Ds.focusEdt(scriptEditor);
						}
					},
					items : [ {
						xtype : 'panel',
						html : '<textarea id="' + edtId + '" name="' + edtId
								+ '"></textarea>'
					} ]
				});
	...
	delaySave : function(isAll, func) {
		...
		Ds.delaySv.delay(110, null, null, [ isAll, func ]);
	},
	...
	finalize : function() {
		seTheme.mon(seTheme, 'activate', function() {
			var o;
			seTheme.menu.items.each(function(n) {
				if (n.text.toLowerCase() == Ds.editorTheme) {
					o = n;
					return false;
				}
			});
			if (o)
				...
				defaultTh.setChecked(true);
		});
		seTheme.menu.items.each(function(n) {
			n.mon(n, 'click', function(o) {
				var s = o.text.toLowerCase();
				Wb.request( {
					url : 'main?xwl=13UABCDZ9IO6',
					params : {
						name : 'wb.ide.editorTheme',
						value : s
					},
					success : function() {
						Ds.editorTheme = s;
						moduleTab.items.each(function(t) {
							t.items.each(function(b) {
								if (b.scriptEditor)
									b.scriptEditor.setOption('theme', s);
							});
						});
					}
				});
			...
			Wb.request( {
				...
				success : function(resp) {
					...
					for (i = 0; i < j; i++) {
						...
						moduleTab.items.each(function(x) {
							if (x.modId === r[i]) {
								x.isModified = false;
								x.close();
							}
	...
	editAbsLayout : function() {
		...
		if (!Ext.isDefined(prop.layout)
				|| selObj.get('xwlCategory').indexOf('tcontrol') != -1) {
			Wb.warning(Wb.format('"{0}" has no absolute layout.', objName));
			return;
		}
		if (prop.layout != 'absolute') {
			if (selObj.childNodes && selObj.childNodes.length) {
				Wb.warning(Wb.format(
						'"{0}" layout property is not set to absolute.',
						objName));
				return;
			}
