				  ...
				  var ev=eval("("+data+")");
				   if(ev.length!=0){
					  for(i=0;i<ev.length;i++){
						  ...
						  spanHtml += "<td  class='ellipsis' width='590px' id='td0"+a+"' style='padding-left: 50px'><a href='javascript:void(0)' onclick=clickFunc('动态预览','"+ev[i].NTC_ID+"','"+_contextPath+"/main?xwl=23WPD5TO7KQ1&showAddWindow=Y&ID="+ev[i].NTC_ID+"')>"+ev[i].NAME+"</a></td>" +
...
//					  document.getElementById("upzhibiao").innerHTML = html;
					  document.getElementById("ssfhvalue").innerHTML="<a style='color:#FFFFFF;' onclick=clickOpenWindow('"+ev[0].PI_CODE+"','') href='javascript:void(0)'>"+parseFloat(ev[0].VALUEDATE).toFixed(2)+"<br>"+ev[0].UNIT+"</a>";
					  document.getElementById("fdlvalue").innerHTML="<a style='color:#FFFFFF;' onclick=clickOpenWindow('"+ev[1].PI_CODE+"','') href='javascript:void(0)'>"+parseFloat(ev[1].VALUEDATE).toFixed(2)+"<br>"+ev[1].UNIT+"</a>";
					  //document.getElementById("sspmvalue").innerHTML=ev[2].VALUEDATE;
					  document.getElementById("dcylvalue").innerHTML="<a style='color:#FFFFFF;' onclick=clickOpenWindow('"+ev[3].PI_CODE+"','') href='javascript:void(0)'>"+parseFloat(ev[3].VALUEDATE).toFixed(2)+ev[3].UNIT+"</a>";
					  }
					  else if(position=="2"){
						  var a=0;
						  var Bar="";
						  ...
						  for(i=0;i<ev.length;i++){
							  ...
							  if(typeof(ev[i].VALUEDATE)!='undefined'){
						           ...
						           spanHtml+="<div class='td1' style='height:25px;'>"
										   +"<span class='tdn'><a "+Bar+">"+ev[i].QUOTA_NAME+"</a></span><span class='tdn2' id='"+ev[i].QUOTA_NAME+"'>"+parseFloat(ev[i].VALUEDATE).toFixed(2)+ev[i].UNIT+"</span>"
										   +"</div>";
						    }
						  }
	...
	$.ajax({
			  ...
			  success: function(data){
				  ...
				  if(ev.length!=0 && typeof(ev[0].YEAR_PLAN)!='undefined'){
					  document.getElementById("年计划发电量").innerHTML=parseFloat(ev[0].YEAR_PLAN)+ev[0].UNIT;
					  document.getElementById("月度计划发电量").innerHTML=parseFloat(ev[0].MONTH_PLAN)+ev[0].UNIT;
					  document.getElementById("年度发电量完成率").innerHTML=(parseFloat(yearCount)/parseFloat(ev[0].YEAR_PLAN)).toFixed(2);
				  }
			  },
			  error: function (data) {
	        }
